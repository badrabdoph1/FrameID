...
describe("services fulfillment", () => {
  it("runs an automatic workflow idempotently through entitlements and activation", async () => {
    ...
    const service = createFulfillmentService({
      ...
      async activateProduct(input) { events.push(`activate:${input.instanceKey}`); return { id: input.instanceKey === "pricing-primary" ? "instance_1" : "instance_2" }; },
      async createSubscription(input) { events.push(`subscribe:${input.billingInterval}`); return { id: "subscription_1" }; },
    });

    await expect(service.start({ acquisitionId: "acq_1", idempotencyKey: "fulfill_1" })).resolves.toMatchObject({ status: "SUCCEEDED", productInstanceId: "instance_1", productInstanceIds: ["instance_1", "instance_2"] });
    expect(events).toEqual(["run:fulfill_1", "acquisition:FULFILLING", "running", "workflow", "grant:pricing_site.access", "activate:pricing-primary", "activate:gallery:acq_1", "subscribe:MONTHLY", "succeeded", "acquisition:FULFILLED"]);
  });
  ...
  it("lets operations complete a waiting manual run through the same entitlement and activation boundary", async () => {
    const events: string[] = [];
    const acquisition = { id: "acq", tenantId: "t", productId: "product", offeringId: "o", workflowKey: "manual_service", workflowVersion: 1, status: "FULFILLING" as const, instanceKey: "manual-primary", additionalActivations: [], billingInterval: "ONE_TIME" as const, capabilities: [{ capabilityKey: "manual.access", capabilityId: null, value: true }] };
    const repository: FulfillmentRepository = {
  ...
  it("re-executes a failed automatic workflow instead of bypassing it with manual completion", async () => {
    const events: string[] = [];
    const acquisition = { id: "acq", tenantId: "t", productId: null, offeringId: "o", workflowKey: "automatic", workflowVersion: 1, status: "FULFILLING" as const, instanceKey: null, additionalActivations: [], billingInterval: "ONE_TIME" as const, capabilities: [] };
    const repository: FulfillmentRepository = {
      async getAcquisition() { return acquisition; },
    ...
    const service = createFulfillmentService({
      repository,
      workflows: createWorkflowRegistry([{ key: "automatic", async execute() { events.push("workflow"); return { status: "COMPLETED" }; } }]),
      async grantEntitlement() {}, async activateProduct() { throw new Error("unused"); }, async createSubscription() { throw new Error("unused"); },
    });
    await expect(service.retry({ runId: "run", idempotencyKey: "retry" })).resolves.toMatchObject({ status: "SUCCEEDED" });
    expect(events).toEqual(["running", "workflow", "succeeded", "FULFILLED"]);
  });

  it("does not execute a workflow when another worker already claimed the run", async () => {
    const repository: FulfillmentRepository = {
      ...
      async markWaiting() { throw new Error("unused"); },
      async markFailed() { throw new Error("unused"); },
      async transitionAcquisition() {},
    };
    let workflowExecutions = 0;
    const service = createFulfillmentService({
      repository,
      workflows: createWorkflowRegistry([{ key: "automatic", async execute() { workflowExecutions += 1; return { status: "COMPLETED" }; } }]),
