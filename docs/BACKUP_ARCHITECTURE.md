...
GitHub هو المصدر الرسمي كذلك لكتالوج النسخ الناجحة. جداول `BackupJob` وسجل إعادة الفهرسة في PostgreSQL مجرد Index قابل لإعادة البناء. عند إقلاع النظام وعند فتح مركز النسخ تقرأ خدمة المصالحة كل Manifests السليمة في الفرعين، وتعيد إنشاء أو تصحيح `BackupJob` وAudit قابل للتتبع، وتحذف من الفهرس السجلات المكتملة التي لم تعد موجودة بعد Retention. العملية تلقائية وIdempotent ولا تحتاج تدخل الأدمن. تفاصيل التشغيل التاريخية غير الموجودة في Manifest لا تُختلق؛ يسجل النظام الحدث الصادق `BACKUP_REINDEXED_FROM_GITHUB`.
