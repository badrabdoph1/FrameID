...
export function getTemplateContentSource(code: string): TemplateContentSource | null {
  ...
  const source: TemplateContentSource = {
    ...
    template,
    theme,
    content: clone(template.starterContent),
  };
...
export function createTemplateProvisioningPayload(
  ...
  return {
    ...
    sections: [
      {
        ...
        sortOrder: sections.hero.sortOrder,
        isVisible: sections.hero.isVisible,
        data: {
      ...
      },
      {
        ...
        sortOrder: sections.gallery.sortOrder,
        isVisible: sections.gallery.isVisible,
        data: {
          title: sections.gallery.title,
      ...
      },
      {
        ...
        sortOrder: sections.packages.sortOrder,
        isVisible: sections.packages.isVisible,
        data: {
          title: sections.packages.title,
      ...
      {
        type: "extras",
        title: sections.extras.title,
        sortOrder: sections.extras.sortOrder,
        isVisible: sections.extras.isVisible,
        data: {
          title: sections.extras.title,
          description: sections.extras.description,
        },
      },
      {
        type: "contact",
        title: sections.contact.title,
        sortOrder: sections.contact.sortOrder,
        isVisible: sections.contact.isVisible,
        data: {
          callToAction: sections.contact.callToAction,
        },
      },
    ],
    contact: {
      ...clone(content.contact),
      callToAction: sections.contact.callToAction,
    },
    ...
    extras: content.extras.map((item) => ({
      ...item,
      price: formatMoney(item.priceAmount, item.currency),
    })),
  ...
  return {
    ...
    metadata: {
      ...
      openGraph: {
        ...
        title: payload.seo.title,
        description: payload.seo.description,
        images: payload.sections[0]?.data.imageUrl
          ? [{ url: String(payload.sections[0].data.imageUrl), alt: payload.site.title }]
          : undefined,
      },
      twitter: {
        ...
        description: payload.seo.description,
        images: payload.sections[0]?.data.imageUrl
          ? [String(payload.sections[0].data.imageUrl)]
          : undefined,
      },
    },
    ...
    sections: Object.fromEntries(
      payload.sections.map((section) => [
        section.type,
        {
          title: section.title,
          description:
            typeof section.data.description === "string" ? section.data.description : null,
    ...
    gallery: payload.gallery.images.map((item) => ({
      id: item.id,
      url: item.url,
      alt: item.alt,
...
export function validateTemplateContentSource(source: TemplateContentSource): void {
  ...
  for (const section of requiredSections) {
    if (!content.sections[section]) {
      throw new Error(`Template ${source.code} is missing ${section} section content`);
    }
  }
