...
function assertValidTransition(current: PaymentStatus, target: PaymentStatus, errorCode: string): void {
  ...
  if (!allowed?.includes(target)) {
    throw new PaymentError(errorCode, `لا يمكن نقل طلب الدفع من ${current} إلى ${target}`);
  }
}

function assertDraftStatus(status: string): void {
  if (status !== "DRAFT") {
    throw new PaymentError("FID-PAY-003", "لا يمكن تعديل طلب الدفع بعد إرساله للمراجعة");
  }
}
...
export type BillingActivationRepository = {
  ...
  getCustomerActivePaymentRequest(tenantId: string): Promise<{
    id: string;
    status: string;
    method: string;
    amount: number;
  ...
  approvePayment(
    ...
    adminNote?: string,
    reviewedAt?: Date,
  ): Promise<{ tenantId: string; subscriptionId: string; planId: string | null }>;
  ...
  getPaymentRequestById(id: string): Promise<{
    id: string;
    status: string;
    tenantId: string;
    subscriptionId: string;
...
export function createBillingActivationService({ repository, now = () => new Date() }: { repository: BillingActivationRepository; now?: () => Date }) {
  return {
    async createDraftPayment(input: {
      ...
      await repository.recordAudit(undefined, input.tenantId, "PAYMENT_DRAFT_CREATED", "PaymentRequest", draft.id, {
        planId: input.planId,
        method: input.method,
    ...
    async submitPayment(paymentRequestId: string) {
      ...
      await repository.recordAudit(undefined, result.tenantId, "PAYMENT_REQUEST_SUBMITTED", "PaymentRequest", paymentRequestId, {
    ...
    async approvePayment(input: { paymentRequestId: string; reviewerId: string; adminName?: string; adminNote?: string }) {
      ...
      if (approved.planId) {
        ...
        if (plan) {
          const interval = plan.billingInterval.toUpperCase();
          periodEnd = addMonths(reviewedAt, interval === "YEARLY" || interval === "ANNUAL" ? 12 : 1);
        }
      }

      await repository.activateSubscription(approved.tenantId, approved.subscriptionId, approved.planId, reviewedAt, periodEnd);
      await repository.recordSubscriptionChange(approved.subscriptionId, payment.subscription.planId, approved.planId, payment.subscription.status, "ACTIVE", "ACTIVATE", input.reviewerId, "تم تفعيل الاشتراك بعد الموافقة على الدفع");
      await repository.addLog(input.paymentRequestId, "APPROVED", input.reviewerId, input.adminName, input.adminNote || "تم قبول طلب الدفع وتفعيل الاشتراك", { activatedAt: reviewedAt.toISOString(), expiresAt: periodEnd?.toISOString() });
      await repository.createNotification(approved.tenantId, "payment_approved", "تم قبول طلب الدفع", "تمت الموافقة على طلب الدفع الخاص بك وتم تفعيل اشتراكك. يمكنك الآن استخدام جميع الميزات.", "high");
      ...
      await repository.recordAudit(input.reviewerId, approved.tenantId, "PAYMENT_APPROVED", "PaymentRequest", input.paymentRequestId, { adminNote: input.adminNote, activatedAt: reviewedAt.toISOString() });
      await repository.recordAudit(input.reviewerId, approved.tenantId, "SUBSCRIPTION_ACTIVATED", "Subscription", approved.subscriptionId, { planId: approved.planId, expiresAt: periodEnd?.toISOString() });
    },
    ...
    async rejectPayment(input: { paymentRequestId: string; reviewerId: string; adminName?: string; reason: string; adminNote?: string }) {
      ...
      await repository.createNotification(rejected.tenantId, "payment_rejected", "تم رفض طلب الدفع", `عذراً، تم رفض طلب الدفع الخاص بك. السبب: ${input.reason}. يرجى مراجعة البيانات وإعادة التقديم.`, "high");
    ...
    async requestReupload(input: { paymentRequestId: string; reviewerId: string; adminName?: string; note: string }) {
      ...
      await repository.createNotification(payment.tenantId, "reupload_requested", "مطلوب إعادة رفع إثبات الدفع", `يرجى إعادة رفع صورة إثبات الدفع مع التعديلات المطلوبة. ملاحظة: ${input.note}`, "high");
