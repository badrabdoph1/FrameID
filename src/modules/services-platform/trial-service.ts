...
export interface TrialRepository {
  ...
  getGrantByIdempotency(tenantId: string, idempotencyKey: string): Promise<{ id: string; status: "ACTIVE"; startsAt: Date; endsAt: Date | null; graceEndsAt: Date | null; usageLimit: number | null } | null>;
...
export function createTrialService(
  ...
  return {
    async start(input: { tenantId: string; policyId: string; idempotencyKey: string }) {
      ...
      if (!policy) throw new Error("Trial policy was not found.");
      const existing = await repository.getGrantByIdempotency(input.tenantId, input.idempotencyKey);
      if (!policy.isActive && !existing) throw new Error("Trial policy is not active.");
      let grant = existing;
      if (!grant) {
        ...
        grant = await repository.createGrant({
