...
export default async function AdminThemesPage() {
  ...
  const themes = await prisma.theme.findMany({ where: { deletedAt: null }, orderBy: [{ status: "desc" }, { updatedAt: "desc" }], select: { id: true, code: true, name: true, category: true, version: true, status: true, updatedAt: true, _count: { select: { sites: true, templates: true } } } });
  return (
    <AdminPageShell badge="المحتوى" title="الثيمات" description="مرجع الثيمات المستخدمة فعليًا وعدد القوالب والمواقع المرتبطة بكل ثيم." breadcrumbs={[{ label: "المحتوى", href: "/admin/content" }, { label: "الثيمات" }]} actions={[{ label: "إدارة القوالب", href: "/admin/templates", icon: Palette }]}>
      <section aria-label="قائمة الثيمات" className="grid gap-3">
        {themes.length === 0 ? <AdminEmptyState title="لا توجد ثيمات" description="أنشئ الثيم من مسار التطوير ثم سيظهر هنا تلقائيًا." icon={Palette} /> : themes.map((theme) => {
          const meta = status[theme.status];
