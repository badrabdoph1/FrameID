...
export async function saveTemplateVisualImageAction(formData: FormData) {
  ...
  try {
    const current = await prisma.template.findFirst({ where: { id, deletedAt: null } });
    if (!current) throw new Error("القالب غير موجود.");

    const previewData: JsonRecord = isRecord(current.previewData) ? { ...current.previewData } : {};
    ...
    if (!useDefault) {
      ...
      nextUrl = (await uploadPlatformTemplateImage(file)).url;
    }
    ...
    if (target === "hero") {
      ...
      if (!Array.isArray(previewData.packages)) throw new Error("لا توجد باقات محفوظة داخل القالب.");
      let found = false;
