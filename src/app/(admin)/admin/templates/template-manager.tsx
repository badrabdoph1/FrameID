      ...
      <section className="rounded-3xl border border-amber-300/20 bg-amber-300/[0.055] p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-black text-[#f3cf73]">بيانات القوالب موحدة</p>
            <h2 className="mt-1 text-lg font-black text-[#fff7e8]">كل القوالب تشترك في نفس المحتوى</h2>
            <p className="mt-1 text-xs font-bold leading-6 text-white/42">جميع القوالب الحالية والجديدة تأخذ البيانات والنصوص من مصدر واحد (اسم المصور، الاستوديو، الوصف). التعديل يتم مرة واحدة فقط. كل قالب يختلف فقط في الثيم والصور والترتيب.</p>
          </div>
        </div>
      </section>

      <section className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-white/10 bg-white/[0.035] p-4">
        <div>
          <p className="text-xs font-black text-[#f3cf73]">إدارة القوالب</p>
          <h2 className="mt-1 text-lg font-black text-[#fff7e8]">القوالب الجاهزة</h2>
          <p className="mt-1 text-xs font-bold leading-6 text-white/42">كل قالب يحدد فقط الثيم والترتيب والصورة — أما المحتوى فمن المصدر المشترك.</p>
        </div>
        ...
        <section className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
          ...
          {showCreate ? (
            <form action={createTemplateAction} className="grid content-start gap-3 rounded-2xl border border-amber-300/20 bg-amber-300/[0.055] p-4 sm:grid-cols-2">
              <div className="sm:col-span-2"><p className="text-xs font-black text-[#f3cf73]">قالب جديد</p><h2 className="mt-1 text-lg font-black text-[#fff7e8]">بيانات القالب الأساسية</h2><p className="mt-1 text-xs font-bold text-white/42">المحتوى (الاسم والوصف) سيتم توريثه من المصدر المشترك تلقائيًا.</p></div>
  ...
  return (
    <article className="min-w-0 rounded-3xl border border-white/10 bg-white/[0.035] p-4">
      ...
      <form action={saveTemplateAction} className="mt-4 grid gap-4">
        ...
        <EditorSection icon={Settings2} title="إعدادات القالب" description="هوية القالب وحالته وترتيبه فقط — المحتوى من المصدر المشترك.">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="الثيم"><select name="themeId" defaultValue={template.theme.id} className={inputClass}>{themes.map((theme) => <option key={theme.id} value={theme.id}>{theme.name} — {theme.code}</option>)}</select></Field>
            <Field label="حالة النشر"><select name="status" defaultValue={template.status} className={inputClass}><option value="PUBLISHED">منشور</option><option value="DRAFT">مسودة</option><option value="ARCHIVED">مؤرشف</option></select></Field>
        ...
        <EditorSection icon={ImageIcon} title="صورة القالب" description="صورة العرض في كتالوج القوالب والصورة المصغرة.">
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="صورة المعاينة"><input name="previewImage" defaultValue={previewImage} className={inputClass} placeholder="رابط الصورة" /></Field>
            <Field label="صورة الغلاف"><input name="coverImage" defaultValue={pickText(template.previewData, ["coverImage"])} className={inputClass} placeholder="رابط صورة الغلاف" /></Field>
          </div>
        </EditorSection>

        <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-gradient-to-br from-[#f3cf73] to-[#d4af37] px-4 text-sm font-black text-[#17120a] shadow-lg transition hover:-translate-y-0.5"><Save className="size-4" /> حفظ التعديلات</button>
      </form>
      ...
      <section className="mt-4 grid gap-2 border-t border-white/8 pt-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <form action={restoreTemplateDefaultsAction} onSubmit={(event) => { if (!window.confirm("سيتم إعادة ضبط القالب. هل أنت متأكد؟")) event.preventDefault(); }}><input type="hidden" name="id" value={template.id} /><ActionButton><RotateCcw className="size-3.5" /> إعادة ضبط</ActionButton></form>
