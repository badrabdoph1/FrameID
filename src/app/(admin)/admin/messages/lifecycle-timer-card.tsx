...
import { CalendarClock, CheckCircle2, ChevronDown, Clock3, Search, Settings2, ShieldCheck, Sparkles } from "lucide-react";
...
export function LifecycleTimerCard({ settings, trialTenants, activeTenants }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-[1.6rem] border border-amber-300/16 bg-[linear-gradient(145deg,rgba(255,255,255,0.05),rgba(255,255,255,0.018))] shadow-[0_24px_80px_rgba(0,0,0,0.22)]">
      <header className="grid gap-3 border-b border-white/10 p-4 lg:grid-cols-[1fr_auto] lg:items-center xl:p-5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <span className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl text-xs font-bold leading-6 text-white/48 lg:text-sm">
              إعداد بسيط: إظهار أو إخفاء كارت العدّاد للعميل، وتحديد أيام التجربة الافتراضية من تاريخ إنشاء الحساب.
            </p>
          </span>
        </div>
        <span className="rounded-2xl border border-white/10 bg-black/18 px-3 py-2 text-xs font-black text-white/48">
          Trial: {trialTenants.length} · Active: {activeTenants.length}
        </span>
      </header>

      <div className="grid gap-4 p-4 xl:p-5">
        <form action={saveLifecycleTimersAction} className="grid gap-4 rounded-[1.35rem] border border-white/10 bg-[#0d1016]/72 p-3 sm:p-4 xl:grid-cols-[1fr_1fr_auto] xl:items-end xl:p-5">
          <PanelMini title="ظهور الكارت للعميل" description="الإيقاف هنا يخفي كارت العدّاد فقط من لوحة العميل.">
            <div className="grid gap-2 sm:grid-cols-2">
              <TogglePill name="trialEnabled" label="إظهار مؤقت التجربة" defaultChecked={settings.trial.enabled} />
              <TogglePill name="subscriptionEnabled" label="إظهار مؤقت الاشتراك" defaultChecked={settings.subscription.enabled} />
            </div>
          </PanelMini>

          <PanelMini title="الأيام الافتراضية للتجربة" description="تُحسب من تاريخ إنشاء العميل، وليس من وقت الحفظ.">
            <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
              <Field label="عدد أيام التجربة الافتراضية">
                <input name="trialDays" type="number" min={1} max={3650} defaultValue={settings.trial.defaultDays} className={inputClass} />
              </Field>
              <label className="flex min-h-11 items-center gap-2 rounded-2xl border border-white/8 bg-white/[0.035] px-3 text-xs font-black text-white/60">
                <input name="trialUseDefault" type="checkbox" defaultChecked={settings.trial.useDefault} />
                تطبيقها على Trial الحاليين
              </label>
            </div>
          </PanelMini>

          <PanelMini title="افتراضي الاشتراك" description="يستخدم عند قبول الدفع أو عند اختيار مدة افتراضية.">
          ...
          <button className="min-h-12 rounded-2xl border border-amber-300/36 bg-gradient-to-br from-[#f3cf73] to-[#d4af37] px-5 text-sm font-black text-[#17120a] shadow-lg shadow-amber-500/10 transition hover:-translate-y-0.5 xl:col-span-3">
            حفظ إعدادات المؤقت
          </button>
        </form>

        <details className="group overflow-hidden rounded-[1.35rem] border border-white/10 bg-black/14">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 text-sm font-black text-[#fff7e8] transition hover:bg-white/[0.035] [&::-webkit-details-marker]:hidden">
            <span className="inline-flex items-center gap-2"><Settings2 className="size-4 text-[#f3cf73]" /> تطبيق يدوي متقدم على العملاء</span>
            <ChevronDown className="size-4 text-white/42 transition group-open:rotate-180" />
          </summary>

          <div className="border-t border-white/10 p-3 sm:p-4">
            <div className="mb-3 grid gap-2 rounded-[1.15rem] border border-white/10 bg-black/16 p-1.5 sm:grid-cols-2">
              <TabButton active={activeTab === "trial"} onClick={() => { setActiveTab("trial"); setAudience("selected"); }} title="عملاء Trial" count={trialTenants.length} icon={CalendarClock} />
              <TabButton active={activeTab === "subscription"} onClick={() => { setActiveTab("subscription"); setAudience("selected"); }} title="المشتركين" count={activeTenants.length} icon={ShieldCheck} />
            </div>

            <form action={applyLifecycleTimerAction} className="grid gap-4 xl:grid-cols-[340px_1fr]">
              ...
              <aside className="grid gap-3 content-start">
                <div className="rounded-3xl border border-amber-300/14 bg-amber-300/[0.055] p-4">
                  ...
                  <h3 className="mt-3 text-lg font-black text-[#fff7e8]">{activeTab === "trial" ? "تعديل تجارب محددة" : "تعديل اشتراكات محددة"}</h3>
                  <p className="mt-1 text-xs font-bold leading-6 text-white/48">
                    الأفضل استخدام الوضع الذكي. التمديد الفردي التفصيلي موجود داخل صفحة العميل من قسم العملاء.
                  </p>
                </div>
                ...
                <div className="grid gap-2 rounded-2xl border border-white/8 bg-black/14 p-3">
                  <ChoiceButton active={timerMode === "keep"} onClick={() => setTimerMode("keep")} title="الوضع الذكي" meta="يحافظ على تاريخ كل عميل الحالي" compact />
                  ...
                  {timerMode === "override" ? (
                    activeTab === "trial" ? (
                      <Field label="أيام التجربة الجديدة من تاريخ التسجيل">
                        <input name="trialDays" type="number" min={1} max={3650} defaultValue={settings.trial.defaultDays} className={inputClass} />
                      </Field>
                    ) : (
                    ...
                    <div className="rounded-2xl border border-emerald-300/14 bg-emerald-300/[0.06] p-3 text-xs font-bold leading-6 text-emerald-100/75">
                      لا يغير التواريخ. عميل فاضله 3 أيام يظل 3 أيام، والاشتراك الدائم يظل دائمًا.
                    </div>
                  )}
                </div>

                <button className="min-h-12 rounded-2xl border border-amber-300/40 bg-gradient-to-br from-[#f3cf73] to-[#d4af37] text-sm font-black text-[#17120a] shadow-lg shadow-amber-500/10 transition hover:-translate-y-0.5">تطبيق على العملاء</button>
              </aside>
              ...
              <section className="grid min-w-0 gap-3">
                ...
                <div className="rounded-2xl border border-white/8 bg-black/12 p-2">
                  ...
                  <div className="grid max-h-[360px] gap-2 overflow-y-auto pr-1 admin-scrollbar lg:max-h-[480px]">
...
function TogglePill({ name, label, defaultChecked }: { name: string; label: string; defaultChecked: boolean }) {
  return (
    <label className="flex min-h-11 items-center gap-2 rounded-2xl border border-white/8 bg-white/[0.035] px-3 text-xs font-black text-white/66">
...
function PanelMini({ title, description, children }: { title: string; description: string; children: ReactNode }) {
  return (
    <section className="grid gap-3 rounded-[1.15rem] border border-white/8 bg-black/16 p-3">
      <span>
