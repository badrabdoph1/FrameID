...
import { Activity, CalendarDays, CheckCircle2, Clock, CreditCard, DollarSign, ExternalLink, FileText, ImageIcon, X, XCircle } from "lucide-react";

import { AdminPageShell } from "@/components/layout/admin-page-shell";
import { StatCard } from "@/components/admin/shared/stat-card";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils/cn";
import { approvePaymentAction, rejectPaymentAction, requestReuploadAction, addPaymentNoteAction } from "@/app/(admin)/admin/payments/actions";
...
export function AdminPaymentsClient({ payments, stats, banner }: Props) {
  ...
  const bannerText = banner === "approved"
    ? "تم قبول الدفع وتفعيل الاشتراك بالمدة المحددة."
    : banner === "rejected"
      ? "تم رفض طلب الدفع."
      : banner === "reupload"
        ? "تم طلب إعادة رفع إثبات الدفع."
        : banner === "note-added"
          ? "تم حفظ الملاحظة."
          : banner === "error"
  ...
  async function submitAction(action: (fd: FormData) => Promise<void>, formData: FormData) {
    setSubmitting(true);
    try {
      await action(formData);
      router.refresh();
    } catch {
      setMessage("فشلت العملية. حاول مرة أخرى.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function confirmApproval() {
    ...
    await submitAction(approvePaymentAction, formData);
    setApproval(null);
  }
  ...
  async function reject(payment: PaymentRequestFull) {
    ...
    if (!reason) {
      setMessage("اكتب سبب الرفض أولًا.");
      return;
    }
    const formData = new FormData();
  ...
  async function requestReupload(payment: PaymentRequestFull) {
    ...
    if (!note) {
      setMessage("اكتب ملاحظة إعادة الرفع أولًا.");
      return;
    }
    const formData = new FormData();
    formData.set("paymentRequestId", payment.id);
  ...
  async function addNote(payment: PaymentRequestFull) {
    ...
    if (!note) {
      setMessage("اكتب الملاحظة أولًا.");
      return;
    }
    const formData = new FormData();
    formData.set("paymentRequestId", payment.id);
  ...
  return (
    <AdminPageShell badge="المال" title="مركز المدفوعات" description="مراجعة إثباتات الدفع وتحديد مدة الاشتراك قبل التفعيل.">
      {(message || bannerText) ? (
        <div className={cn("rounded-2xl border px-4 py-3 text-sm font-black", banner === "error" || message ? "border-red-500/20 bg-red-500/10 text-red-300" : "border-emerald-500/20 bg-emerald-500/10 text-emerald-300")}>{message || bannerText}</div>
      ) : null}
    ...
    <div className="fixed inset-0 z-[2147483000] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
      <section className="w-full max-w-xl overflow-hidden rounded-[2rem] border border-amber-300/18 bg-[#111720] text-[#fff7e8] shadow-2xl">
        <header className="flex items-start justify-between gap-3 border-b border-white/10 bg-amber-300/[0.06] p-5">
          <div>
            <p className="text-xs font-black text-[#f3cf73]">تأكيد تفعيل الاشتراك</p>
            <h2 className="mt-1 text-xl font-black">اختر مدة الاشتراك قبل قبول الدفع</h2>
            <p className="mt-1 text-sm font-bold text-white/48">العميل: {approval.payment.tenant.displayName}</p>
          </div>
        ...
        <div className="grid gap-4 p-5">
          <div className="grid gap-2 sm:grid-cols-2">
            {lifecycleDurationOptions.map((option) => (
              <label key={option.value} className={cn("flex min-h-12 cursor-pointer items-center gap-3 rounded-2xl border px-3 text-sm font-black transition", approval.durationPreset === option.value ? "border-amber-300/45 bg-amber-300/12 text-[#f3cf73]" : "border-white/10 bg-white/[0.035] text-white/58 hover:border-amber-300/18")}>
                <input type="radio" name="duration" checked={approval.durationPreset === option.value} onChange={() => setApproval({ ...approval, durationPreset: option.value })} />
                {option.label}
              </label>
            ))}
          </div>
          ...
          {approval.durationPreset === "custom" ? (
            <label className="grid gap-1.5">
              <span className="text-xs font-black text-white/42">المدة المخصصة بالأيام</span>
              <input type="number" min={1} max={3650} value={approval.customDays} onChange={(event) => setApproval({ ...approval, customDays: Number(event.target.value) || 30 })} className="min-h-11 rounded-2xl border border-white/10 bg-black/18 px-3 text-sm font-black text-white outline-none focus:border-amber-300/45" />
            </label>
          ) : null}

          <div className="rounded-2xl border border-white/10 bg-black/18 p-4">
            <p className="text-xs font-black text-white/38">تاريخ الانتهاء المتوقع</p>
            <p className="mt-1 text-lg font-black text-[#f3cf73]">{expectedEnd ? formatDate(expectedEnd) : "اشتراك دائم بدون تاريخ انتهاء"}</p>
          </div>

          <label className="grid gap-1.5">
            <span className="text-xs font-black text-white/42">ملاحظة داخلية اختيارية</span>
            <textarea rows={3} value={approval.adminNote} onChange={(event) => setApproval({ ...approval, adminNote: event.target.value })} className="rounded-2xl border border-white/10 bg-black/18 px-3 py-2 text-sm font-bold text-white outline-none focus:border-amber-300/45" />
          </label>
        </div>

        <footer className="grid gap-2 border-t border-white/10 p-5 sm:grid-cols-2">
          <button onClick={onCancel} className="min-h-12 rounded-2xl border border-white/10 bg-white/[0.04] text-sm font-black text-white/62">إلغاء</button>
          <button disabled={submitting} onClick={onConfirm} className="min-h-12 rounded-2xl bg-gradient-to-br from-[#f3cf73] to-[#d4af37] text-sm font-black text-[#17120a] disabled:opacity-50">تأكيد التفعيل</button>
        </footer>
      </section>
    </div>
  );
