...
export default async function AdminBackupsPage({ searchParams }: Props) {
  ...
  const viewModel = await buildBackupCenterViewModel({ prisma: prisma as never, backupRoot });
  ...
  return (
    <AdminPageShell badge="النسخ الاحتياطي" title="Backup Center" description="Data Backup لقاعدة البيانات فقط، وFull Backup لقاعدة البيانات مع كل uploads. كود المنصة مصدره GitHub وليس النسخ الاحتياطية.">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="آخر Migration" value={jobs.find((job) => job.type === "FULL") ? formatDate(jobs.find((job) => job.type === "FULL")!.createdAt) : "لم يتم"} />
        <Metric label="حجم النسخ" value={formatBytes(viewModel.backupSummary.storageUsedBytes)} />
