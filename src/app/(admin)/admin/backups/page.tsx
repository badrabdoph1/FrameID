...
export default async function AdminBackupsPage({ searchParams }: Props) {
  ...
  const backupCenter = (await repository.getBackupCenter()) as {
  ...
  const settings = SUPPORTED_BACKUP_TYPES.map((type) => {
    ...
    return {
      ...
      schedule: existing?.schedule ?? policy.schedule,
      retentionCount: existing?.retentionCount ?? policy.retentionCount,
      lastRunAt: existing?.lastRunAt ?? null,
      nextRunAt: existing?.nextRunAt ?? null,
    };
  });
  const jobs = backupCenter.jobs.filter((job) =>
    SUPPORTED_BACKUP_TYPES.includes(job.type as SupportedBackupType)
  );
  const restores = backupCenter.restores.filter((item) =>
    SUPPORTED_BACKUP_TYPES.includes(item.type as SupportedBackupType)
  );
  ...
  return (
    <AdminPageShell
      ...
      description="إدارة نسخ قاعدة البيانات والملفات، التحقق منها واستعادتها من نقطة عمل واحدة."
    >
      <Feedback params={params} />

      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="آخر نسخة" value={lastBackup ? formatDate(lastBackup.createdAt) : "لم يتم"} />
        <Metric label="آخر استعادة" value={lastRestore ? formatDate(lastRestore.createdAt) : "لم يتم"} />
        <Metric label="النسخ السليمة" value={completed} tone="success" />
        <Metric label="المساحة المسجلة" value={formatBytes(storageUsedBytes)} tone={failed ? "warning" : "default"} />
      </section>

      <WorkspaceSection title="إنشاء نسخة" description="اختر نسخة بيانات فقط أو نسخة كاملة تشمل الملفات.">
        <div className="grid gap-3 md:grid-cols-2">
          {SUPPORTED_BACKUP_TYPES.map((type) => (
            <form key={type} action={runBackupAction} className="rounded-2xl border border-white/[0.07] bg-black/20 p-4">
              ...
              <button className="mt-4 w-full rounded-xl bg-[#f3cf73] px-4 py-2.5 text-sm font-black text-[#17120a]">
                إنشاء النسخة
              </button>
            </form>
          ))}
      ...
      <WorkspaceSection title="النسخ الحالية" description="كل نسخة تظهر مرة واحدة مع حالتها وكل إجراءاتها.">
        <div className="space-y-3">
          {jobs.length === 0 ? (
            ...
            jobs.map((job) => {
              ...
              const ready = job.status === "COMPLETED" && artifactId;
              return (
                  ...
                  <details className="mt-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <form action={restoreBackupAction}>
                        <input type="hidden" name="backupId" value={artifactId ?? ""} />
        ...
        <div className="mt-5 space-y-5">
          <div>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-black text-white">الجدولة والاحتفاظ</h2>
                <p className="mt-1 text-xs font-bold text-white/40">تعديل الإعدادات عند الحاجة فقط.</p>
              </div>
              ...
              {settings.map((setting) => (
                <form key={setting.type} action={updateBackupSettingsAction} className="rounded-2xl border border-white/[0.06] bg-black/20 p-4">
                  ...
                  <div className="mb-3 flex justify-between gap-3">
                    <h3 className="text-sm font-black text-white">{BACKUP_TYPE_LABELS[setting.type]}</h3>
                    <AdminStatusBadge tone={setting.enabled ? "success" : "default"}>{setting.enabled ? "مفعل" : "متوقف"}</AdminStatusBadge>
                  </div>
                  <div className="grid gap-3 md:grid-cols-3">
                    <Field label="الحالة"><select name="enabled" defaultValue={setting.enabled ? "true" : "false"} className="control"><option value="true">مفعل</option><option value="false">متوقف</option></select></Field>
                    <Field label="Cron"><input name="schedule" defaultValue={setting.schedule} className="control font-mono" /></Field>
                    <Field label="عدد النسخ"><input name="retentionCount" type="number" min="1" max="100" defaultValue={setting.retentionCount} className="control" /></Field>
                  </div>
          ...
          <div className="rounded-2xl border border-violet-400/15 bg-violet-500/5 p-4">
            ...
            <p className="mt-2 text-xs font-bold leading-6 text-white/45">تُستخدم عند نقل المنصة؛ الكود مصدره GitHub والبيانات والملفات مصدرها النسخة الكاملة.</p>
...
function WorkspaceSection({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <section className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5"><div className="mb-4"><h2 className="text-base font-black text-[#fff7e8]">{title}</h2><p className="mt-1 text-xs font-bold text-white/40">{description}</p></div>{children}</section>;
}
