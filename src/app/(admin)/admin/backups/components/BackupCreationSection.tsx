    ...
    <section className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs font-bold text-white/40">
          اختر النوع واضغط الزر. لا تُعد النسخة مكتملة إلا بعد رفعها والتحقق منها على فرع النسخ الرسمي في GitHub.
        </p>
      </div>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {SUPPORTED_BACKUP_TYPES.map((type) => (
          <PendingForm
            ...
            className="flex flex-col rounded-2xl border border-white/[0.07] bg-black/20 p-4"
          >
            ...
            {latestPerType[type] ? (
              <p className="mt-3 rounded-lg border border-emerald-500/10 bg-emerald-500/5 px-2.5 py-1.5 text-[10px] font-bold text-emerald-300/80">
                آخر نسخة: {latestPerType[type]}
              </p>
            ) : (
              <p className="mt-3 rounded-lg border border-dashed border-white/10 px-2.5 py-1.5 text-[10px] font-bold text-white/25">
                لا توجد نسخة سابقة
              </p>
            )}
            <PendingButton
