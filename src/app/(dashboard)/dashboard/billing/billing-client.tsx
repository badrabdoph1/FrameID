      ...
      {methods.map((method) => {
        ...
        return (
          <form key={method.id} action={action} onSubmitCapture={() => onMethodSelect(method.id)}>
            ...
            <button type="submit" disabled={disabled} className={`min-h-28 w-full rounded-2xl border p-4 text-right transition ${selected ? "border-amber-400/60 bg-amber-500/10" : "border-white/[0.08] bg-white/[0.025] hover:border-white/20"} ${disabled ? "cursor-not-allowed opacity-60" : ""}`}>
