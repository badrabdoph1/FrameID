...
export default async function ServiceProductPage({ params, searchParams }: { params: Promise<{ productCode: string }>; searchParams: Promise<{ attribution?: string }> }) {
  ...
  const eligibilityContext = await buildPrismaEligibilityContext(prisma, session.tenant.id);
  ...
  return (
    <div className="grid gap-5" dir="rtl">
      <ServicesEventBeacon event={{ name: "offering.viewed", idempotencyKey: `product-view:${product.id}:${session.tenant.id}:${new Date().toISOString().slice(0, 10)}`, productId: product.id, attributionId: query.attribution }} />
      ...
      <section className="grid gap-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {product.offerings.map((offering) => {
            const trial = offering.trialPolicies[0];
            return <article key={offering.id} className="flex flex-col rounded-[1.6rem] border border-white/9 bg-white/[0.035] p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><p className="text-[0.65rem] font-black uppercase tracking-wider text-white/30">{offering.type.replaceAll("_", " ")}</p><h3 className="mt-2 text-xl font-black text-white">{offering.name}</h3></div>{offering.displayPrice ? <strong className="whitespace-nowrap text-lg font-black text-[#f3cf73]">{money(offering.displayPrice.amount, offering.displayPrice.currency)}</strong> : null}</div><p className="mt-3 text-sm font-bold leading-6 text-white/43">{offering.description || offering.shortDescription}</p>
              <div className="mt-5 grid gap-2">{offering.capabilities.map((item) => <div key={item.capabilityId} className="flex items-center gap-2 text-sm font-bold text-white/55"><BadgeCheck className="size-4 text-emerald-300" /><span>{item.capabilityKey}</span><span className="mr-auto text-xs text-white/30">{typeof item.value === "number" ? item.value : ""}</span></div>)}{offering.bundleComponents.map((item) => <div key={item.offeringId} className="flex items-center gap-2 text-sm font-bold text-white/55"><Boxes className="size-4 text-blue-300" />{item.offeringName}</div>)}</div>
