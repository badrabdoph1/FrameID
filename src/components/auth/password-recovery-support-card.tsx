...
import { toWhatsappHref } from "@/modules/support/support-settings";

export const PASSWORD_RECOVERY_SUPPORT_MESSAGE =
  "مقدرش ارجع لك كلمة المرور دلوقتي. كلم الدعم الفني عشان يساعدك.";

export function PasswordRecoverySupportCard({ message }: { message?: string }) {
  const whatsappHref = toWhatsappHref("01038434472", "مرحبًا، لا أستطيع استرداد كلمة المرور في FrameID وأحتاج دعم فني.");
  ...
  return (
    <div className="mb-4 overflow-hidden rounded-[1.35rem] border border-emerald-400/22 bg-[linear-gradient(135deg,rgba(16,185,129,0.12),rgba(255,255,255,0.035))] p-4 shadow-[0_16px_42px_rgba(0,0,0,0.12)]">
      <p className="text-sm font-black leading-7 text-foreground">
        {message || PASSWORD_RECOVERY_SUPPORT_MESSAGE}
      </p>
      <a
        href={whatsappHref}
        target="_blank"
