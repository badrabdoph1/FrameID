...
import { BriefcaseBusiness, Check, ClipboardCopy, LayoutDashboard, MessageSquareMore, RefreshCw } from "lucide-react";
    ...
    <main className="relative grid min-h-[70dvh] place-items-center overflow-hidden bg-background px-4 py-12 text-foreground">
      ...
      <section className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-border/80 bg-card/95 p-6 text-center shadow-[0_28px_90px_rgba(16,16,16,0.11)] sm:p-9" aria-labelledby="dashboard-error-title">
        ...
        <p className="mx-auto mt-4 max-w-md text-balance text-sm font-medium leading-7 text-muted-foreground sm:text-base">
          واجهتنا مشكلة أثناء تحميل لوحة التحكم. جرّب إعادة التحميل، ولو المشكلة استمرت بلّغ الفريق التقني وهنحلها في أسرع وقت.
        </p>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          ...
          <button type="button" onClick={() => void copyErrorDetails()} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-signal/30 bg-signal/8 px-5 text-sm font-bold text-signal transition hover:-translate-y-0.5 hover:bg-signal/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal/40 sm:col-span-2">
