...
import { clientSections } from "@/modules/admin/navigation"
...
export function ClientMobileNav() {
  ...
  const topLevelLinks = clientSections.map((s) => ({
    href: s.links[0]?.href ?? "/dashboard",
    label: s.title,
    Icon: s.icon,
  }))
    ...
    <div className="admin-mobile-nav-shell">
      ...
      <div className={`admin-mobile-menu ${mobileMenuOpen ? "open" : ""}`}>
        ...
        <div className="admin-mobile-search">
          <Search size={16} />
          <input type="text" placeholder="ابحث في لوحتك…" />
          <button>بحث</button>
        </div>
