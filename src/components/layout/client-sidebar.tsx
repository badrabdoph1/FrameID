...
import { clientSections, type AdminSection } from "@/modules/admin/navigation"
...
export function ClientSidebar() {
  ...
  const [activeSection, setActiveSection] = useState<string | null>(() => {
    const found = clientSections.find((section) =>
      section.links.some(
        (link) => pathname === link.href || pathname?.startsWith(link.href + "/")
      )
    )
    return found?.id ?? clientSections[0]?.id ?? null
  })

  const currentSections = clientSections.filter((s) => s.id === activeSection)

  return (
    <aside className="dashboard-sidebar admin-scrollbar">
      <Link href="/dashboard" className="admin-brand" style={{ textDecoration: "none" }}>
      ...
      <Link href="/dashboard/search" className="sidebar-search-link">
        <Search size={15} />
        ابحث في لوحتك…
        <kbd>⌘K</kbd>
      </Link>
