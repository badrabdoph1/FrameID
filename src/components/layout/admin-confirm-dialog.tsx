...
export function AdminConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="admin-scale-in relative z-10 w-full max-w-sm rounded-xl border border-white/8 bg-[#0a0a0a] p-6 shadow-2xl">
        <button onClick={onClose} className="absolute left-4 top-4 flex size-7 items-center justify-center rounded-md text-white/30 transition hover:bg-white/6 hover:text-white/60">
          <X className="size-4" />
        </button>
        ...
        <div className={cn(
          ...
          variant === "default" && "bg-white/6",
        )}>
          <AlertTriangle className={cn("size-6", variant === "danger" && "text-red-400", variant === "warning" && "text-amber-400", variant === "default" && "text-white/40")} />
        </div>
