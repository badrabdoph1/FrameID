...
export function TemplateLivePreview({ template, compact = false }: TemplateLivePreviewProps) {
  const src = `/templates/${template.code}/preview?embed=1#gallery`;
  const desktopScale = compact ? 0.255 : 0.44;
  const mobileScale = compact ? 0.17 : 0.24;
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-[1.1rem] border border-black/10 bg-black shadow-2xl", compact ? "h-[196px]" : "h-[340px]")}>
      ...
      <div className={cn("absolute bottom-4 left-4 overflow-hidden rounded-[1.1rem] border border-white/18 bg-black shadow-[0_18px_60px_rgba(0,0,0,.34)]", compact ? "h-[132px] w-[72px]" : "h-[188px] w-[104px]")}>
